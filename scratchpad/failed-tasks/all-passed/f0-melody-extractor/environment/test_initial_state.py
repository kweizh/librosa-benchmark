...
def test_melody_json_not_yet_created():
    melody_path = os.path.join(WORKSPACE, "melody.json")
    assert not os.path.exists(melody_path), (
        f"Expected {melody_path} to NOT exist before the task starts; the agent must create it."
    )
