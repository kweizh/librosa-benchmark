...
def test_vocal_wav_not_yet_created():
    assert not os.path.exists(VOCAL_WAV), (
        f"Expected {VOCAL_WAV} to NOT exist before the task starts; the agent must create it."
    )


def test_accompaniment_wav_not_yet_created():
    assert not os.path.exists(ACCOMP_WAV), (
        f"Expected {ACCOMP_WAV} to NOT exist before the task starts; the agent must create it."
    )
