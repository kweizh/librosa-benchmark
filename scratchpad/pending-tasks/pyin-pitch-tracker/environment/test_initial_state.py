...
def test_input_wav_is_monophonic_audio():
    ...
    assert 4.0 <= duration <= 20.0, (
        f"Expected input audio duration roughly between 4s and 20s, got {duration:.3f}s."
    )


def test_pitch_track_json_not_yet_created():
    output_path = os.path.join(WORKSPACE, "pitch_track.json")
    assert not os.path.exists(output_path), (
        f"Expected {output_path} to NOT exist before the task starts; the agent must create it."
    )
